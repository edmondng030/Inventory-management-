...
可在桌面及手機瀏覽器運行的本機庫存管理系統。介面以繁體中文為主，資料持久儲存在 SQLite，不依賴付費雲端服務。
...
- 手機盤點：後置鏡頭 Barcode／QR、圖片 Barcode、Tesseract OCR、手動及 mock scanner
...
欄位：SKU、Label Code、Item Name、Description、Category、Quantity、Unit、Location、Minimum Stock、Status、Remark。Item Name 必填，SKU 或 Label Code 至少一項必填且唯一。Quantity 與 Minimum Stock 必須為非負整數。Status 支援 Checked、Unchecked、Missing、Damaged、Low Stock。

到「Excel 匯入」拖放檔案，選擇 Sheet、檢查 mapping 與預覽，確認後才提交。相同 SKU 或 Label Code 會更新，否則新增。錯誤可下載。右上角可匯出全部；session 卡可匯出指定批次。

samples/inventory-sample.xlsx 包含新增與更新；samples/inventory-with-errors.xlsx 包含負數、缺少識別及重複資料。
...
- 重複資料在預覽以不分大小寫檢查；SQLite unique 約束保護最終資料。
