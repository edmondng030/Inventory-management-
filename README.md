...
桌面與手機可用左上選單按鈕展開／收合側邊選單，選單內亦有收合按鈕並支援 Escape。庫存列表預設為「全部（含已封存）」，搜尋忽略大小寫並包含編號、PO、描述、備註、位置及部門名稱；按「搜尋全部部門及封存項目」可保留搜尋字詞並解除其他篩選。封存只停用項目，不會從全項目搜尋移除。盤點批次可展開已盤點項目的目前完整資料和本批次盤點紀錄；未把目前 item 欄位當作歷史快照。相機確認及 Dashboard 仍使用使用中資產，封存項目需先還原才可盤點。
