...
export async function currentUser() { const token=(await cookies()).get(COOKIE)?.value; if(!token)return null; const session=await db.authSession.findUnique({where:{tokenHash:digest(token)},include:{user:{include:{department:true}}}}); return session&&session.expiresAt>new Date()&&session.user.active&&!session.user.deletedAt?session.user:null; }
